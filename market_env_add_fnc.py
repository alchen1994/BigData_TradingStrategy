...
class MarketEnv(gym.Env):
    ...
    def __init__(self, dir_path, target_codes, sudden_death_rate=0.5, test = False, startAssetValue = 5000, current_asset = {'cash':5000, 'stock':0}, finalIndex = 1997):
        ...
        for code in target_codes:
            ...
            for line in f:
                if line.strip() != "":
                    vix, openPrice, high, low, close, volume = line.strip().split(",")
                    # print(dt)
#
                    vix = float(vix)
                    ...
                    if lastClose > 0 and close > 0 and lastVolume > 0 and lastvix!=0:
                        # date_ = (date - lastDate)
    ...
    def _step(self, action):
        ...
        vix = self.stock_data[self.currentIndex][-1]
        vix_change = self.stock_data[self.currentIndex+1][-1] - vix
        ...
        if self.actions[action] == "Buy":
            ...
            self.reward = (self.current_asset_value - self.last_asset_value) + price_change * trading_amount- self.current_asset['stock']-vix_change/100 * trading_amount
            #10
