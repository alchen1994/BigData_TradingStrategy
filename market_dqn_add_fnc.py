...
import random
import copy
from market_env_add_fnc import MarketEnv
from market_model_builder_add import MarketDeepQLearningModelBuilder
...
class DeepQ:
    ...
    def train(self, code_stocks, max_episode=40, verbose=0):

        history = open('./record/history_add_fnc.txt', 'a')
        ...
        for e in range(max_episode):
            ...
            with open("./record/train_loss_add_fnc.txt","a") as file:
    ...
    def predict(self,code_stocks):
        if os.path.exists('./record/test_history_add_fnc.txt'):
            os.remove('./record/test_history_add_fnc.txt')

        history = open('./record/test_history_add_fnc.txt', 'a')
...
if __name__ == "__main__":
    ...
    if os.path.exists("./record/train_loss_add_fnc.txt"):
        os.remove("./record/train_loss_add_fnc.txt")

    if os.path.exists("./record/history_add_fnc.txt"):
        os.remove("./record/history_add_fnc.txt")
    # print( s_and_p)
    # exit()
    # s_and_p = ['MMM']
    # [x for x in a if x not in [2, 3, 7]]
    
    env = MarketEnv(dir_path="./semantic_data/add_fnc/", target_codes=s_and_p, sudden_death_rate=0.3, finalIndex = 370) #1259
    pg = DeepQ(env, gamma=0.80, model_file_name="./model/model_ml3_add_fnc.h5")
    ...
    env = MarketEnv(dir_path="./semantic_data/add_fnc/",target_codes=s_and_p_test, test = True, sudden_death_rate=0.3, finalIndex = 131)
    test_obj = DeepQ(env, gamma=0.80, model_file_name="./model/model_ml3_add_fnc.h5", test = True)
    test_obj_random = DeepQ(env, gamma=0.80, model_file_name="./model/model_ml3_add_fnc.h5", test = True, random = True)
    ...
    plt.plot(s_and_p_test,reward_stock)
    plt.plot(s_and_p_test,reward_stock_random)
    # plt.plot(s_and_p_test,reward_collect)
    # plt.plot(s_and_p_test,reward_collect_random)
    plt.title("DQN_add_fnc vs Random")
    plt.xlabel("stock")
    plt.ylabel("return")
    plt.legend(['DQN_add_fnc', 'Random'], loc='upper left')
    plt.show()
...
####################		uncomment the following if test  		###################
    ...
    plot_train_loss("./record/train_loss_add_fnc.txt")
    plot_train_reward("./record/history_add_fnc.txt")
    exit()
