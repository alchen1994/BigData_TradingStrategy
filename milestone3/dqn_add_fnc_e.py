...
import copy
import csv
from env_add_fnc_e import MarketEnv
...
class DeepQ:
    ...
    def train(self, code_stocks, max_episode=100, verbose=0):
        ...
        history = open('./record/history_fnc.txt', 'a')
        ...
        for e in range(max_episode):
            ...
            with open("./record/train_loss_fnc.txt","a") as file:
...
if __name__ == "__main__":
    ...
    if os.path.exists("./record/train_loss_fnc.txt"):
        os.remove("./record/train_loss_fnc.txt")

    if os.path.exists("./record/history_fnc.txt"):
        os.remove("./record/history_fnc.txt")
    
    env = MarketEnv(dir_path="./semantic_data/add_fnc/", target_codes=s_and_p, sudden_death_rate=0.3, finalIndex = 370) #370
    pg = DeepQ(env, gamma=0.80, model_file_name="./model/fnc_e.h5")
    
    for stock in s_and_p:
        pg.train(code_stocks = stock)

    
    env = MarketEnv(dir_path="./semantic_data/add_fnc/",target_codes=s_and_p_test, test = True, sudden_death_rate=0.3, finalIndex = 131) #131
    test_obj = DeepQ(env, gamma=0.80, model_file_name="./model/fnc_e.h5", test = True)
