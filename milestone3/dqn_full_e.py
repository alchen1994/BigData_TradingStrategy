...
import copy
import csv
from env_full_e import MarketEnv
...
class DeepQ:
    ...
    def train(self, code_stocks, max_episode=100, verbose=0):
        ...
        history = open('./record/history_e.txt', 'a')
...
if __name__ == "__main__":
    ...
    if os.path.exists("./record/train_loss_e.txt"):
        os.remove("./record/train_loss_e.txt")

    if os.path.exists("./record/history_e.txt"):
        os.remove("./record/history_e.txt")
    
    env = MarketEnv(dir_path="./semantic_data/noadd/", target_codes=s_and_p, sudden_death_rate=0.3, finalIndex = 370) #370
    pg = DeepQ(env, gamma=0.80, model_file_name="./model/full_e.h5")
    ...
    test_obj = DeepQ(env, gamma=0.80, model_file_name="./model/full_e.h5", test = True)
