...
if __name__ == "__main__":
    ...
    env = MarketEnv(dir_path="./split_data/train/", target_codes=s_and_p, sudden_death_rate=0.3, finalIndex = 997) #370
    pg = DeepQ(env, gamma=0.80, model_file_name="./model/full.h5")
    ...
    with open('./split_data/test/AMZN.csv','r') as ifile:
    ...
    # s_and_p_test = sorted(['IBM','GOOGL','FB', 'AMZN','AAPL','MSFT','BBY','JPM','NDAQ','NVDA'])
    
    env = MarketEnv(dir_path="./split_data/test/",target_codes=s_and_p_test, test = True, sudden_death_rate=0.3, finalIndex = 257) #131
    test_obj = DeepQ(env, gamma=0.80, model_file_name="./model/full.h5", test = True)
    test_obj_random = DeepQ(env, gamma=0.80, model_file_name="./model/full.h5", test = True, random = True)
    ...
    for stock in s_and_p_test:
        plt.plot(date_new, returns[stock])
    ...
    #         return_1, acts, each_reward = test_obj_random.predict(stock,return_1,acts)
