...
import random
import copy
from market_env_add_s import MarketEnv
...
class DeepQ:
    ...
    def train(self, code_stocks, max_episode=40, verbose=0):
        ...
        history = open('./record/history_add_s.txt', 'a')
        ...
        for e in range(max_episode):
            ...
            with open("./record/train_loss_add_s.txt","a") as file:
    ...
    def predict(self,code_stocks):
        if os.path.exists('./record/test_history_add_s.txt'):
            os.remove('./record/test_history_add_s.txt')

        history = open('./record/test_history_add_s.txt', 'a')
...
if __name__ == "__main__":
    ...
    if os.path.exists("./record/train_loss_add_s.txt"):
        os.remove("./record/train_loss_add_s.txt")

    if os.path.exists("./record/history_add_s.txt"):
        os.remove("./record/history_add_s.txt")
    # print( s_and_p)
    # exit()
    # s_and_p = ['MMM']
    ...
    pg = DeepQ(env, gamma=0.80, model_file_name="./model/model_ml3_add_s.h5")
    ...
    test_obj = DeepQ(env, gamma=0.80, model_file_name="./model/model_ml3_add_s.h5", test = True)
    test_obj_random = DeepQ(env, gamma=0.80, model_file_name="./model/model_ml3_add_s.h5", test = True, random = True)
    ...
    plt.title("DQN_add_semantic vs Random")
    plt.xlabel("stock")
    plt.ylabel("return")
    plt.legend(['DQN_add_semantic', 'Random'], loc='upper left')
    plt.show()
...
####################		uncomment the following if test  		###################
    ...
    plot_train_loss("./record/train_loss_add_s.txt")
    plot_train_reward("./record/history_add_s.txt")
    exit()
